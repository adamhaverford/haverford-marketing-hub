...
function CommentThread({ comments, designId }: { comments: Comment[]; designId: string }) {
  ...
  function handleAddComment() {
    ...
    startTransition(async () => {
      await addDesignComment(designId, text)
    })
  }
...
function DesignCard({ design, role }: { design: Design; role: 'marketing' | 'stakeholder' }) {
  ...
  function handleApprove() {
    startTransition(async () => {
      if (isApproved) {
        await setDesignStatus(design.id, 'pending')
      } else {
        await setDesignStatus(design.id, 'approved')
      }
    })
  }
    ...
    <div className={`rounded-xl border p-4 ${borderClass} ${!design.is_current && isDeclined ? 'opacity-60' : ''}`}>
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {isApproved && design.actioned_by_profile && (
            <p className="text-xs text-green-600 font-medium mt-1">
              Approved by {design.actioned_by_profile.full_name ?? 'Unknown'}
              {design.actioned_at && <> · {timeAgo(design.actioned_at)}</>}
            </p>
          )}
          {isDeclined && design.actioned_by_profile && (
            <p className="text-xs text-red-600 font-medium mt-1">
              Declined by {design.actioned_by_profile.full_name ?? 'Unknown'}
              {design.actioned_at && <> · {timeAgo(design.actioned_at)}</>}
            </p>
          )}
        </div>
        ...
        <div className="flex items-start gap-2 flex-shrink-0">
          {role === 'stakeholder' && design.is_current && (
            <>
              <button
                onClick={handleApprove}
                disabled={isPending || isDeclined}
                title={isApproved ? 'Undo approval' : 'Approve design'}
              ...
              <button
                onClick={() => isDeclined ? handleUndecline() : setShowDeclineInput(!showDeclineInput)}
                disabled={isPending || isApproved}
                title={isDeclined ? 'Undo decline' : 'Decline design'}
                className={`flex items-center justify-center w-8 h-8 rounded-full border-2 transition-all disabled:opacity-40 ${
                  isDeclined
...
export default function DesignReview({ brandId, month, type, designs, role }: DesignReviewProps) {
  ...
  const showUploadButton =
    role === 'marketing' && (!currentDesign || currentDesign.status === 'declined')
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Current design */}
      {currentDesign && (
        <DesignCard design={currentDesign} role={role} />
      )}
